...
def get_data(batch_size, device):
    ...
    #     def __init__(self, mu, sigma, sde_type):
    #         super().__init__()
    #         self.register_buffer('mu', torch.as_tensor(mu))
    #         self.register_buffer('sigma', torch.as_tensor(sigma))
    #         self.sde_type = sde_type
    ...
    # elif sde_prcs == "gbm":
    #     sde_gen = GeometricBrownianMotionSDE(mu=0.1, sigma=0.2, sde_type=sde_type, noise_type=noise_type).to(device)
    # else:
        # raise ValueError(f"Invalid sde_prcs: {sde_prcs}")
...
###################
def plot(ts, generator, dataloader, num_plot_samples, plot_locs, config_num):
    # Get samples
    ...
    for i, prop in enumerate(plot_locs):
        ...
        plt.tight_layout()
        # plt.show()
        plt.savefig(f'brian_testing/output/{config_num}_hist_{i}.png')
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig(f'brian_testing/output/{config_num}_samples.png')
...
def run_main(
    ...
    if optimiser == "adadelta":
        ...
        discriminator_optimiser = torch.optim.Adam(discriminator.parameters(), lr=discriminator_lr, weight_decay=weight_decay)
    else:
        raise ValueError(f"Invalid generator_optimiser: {optimiser}")
    ...
    with open(f'brian_testing/output/{config_num}_loss_results.txt', 'w') as f:
